...
function renderBuildCode(b) {
  ...
  el.innerHTML=`<div class="build-tabs">${hero.builds.map((x,i)=>`<button class="build-tab${i===state.buildIndex?' active':''}" type="button" data-build-index="${i}">${esc(loc(x.label))}</button>`).join('')}</div>${dateHtml}<p class="build-desc">${esc (loc(b.summary))}</p>
...
function renderDetail() {
  ...
  els.detailView.innerHTML = `
    ...
    <section class="detail-block">
      <h3 class="section-title">${t('gameplay')}</h3>

      <p class="hero-gameplay">
        ${esc(loc(h.gameplay))}
      </p>
    ...
    <section class="detail-block">
      <h3 class="section-title">${t('tips')}</h3>
